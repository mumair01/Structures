...
void UArray_resize(T ray_struct, int new_length)
{
	...
	else
	{
		if (new_length > ray_struct->len)
			newElems = memcpy(calloc(new_length, ray_struct->elemSize), ray_struct->elems, ray_struct->len * ray_struct->elemSize);
		else
			newElems = memcpy(calloc(new_length, ray_struct->elemSize), ray_struct->elems, new_length * ray_struct->elemSize);
		free(ray_struct->elems);
...
T UArray_copy(T ray_struct, int new_length)
{
	...
	if (new_arr->len > ray_struct->len && ray_struct->len > 0)
		...
		memcpy(new_arr->elems, ray_struct->elems, new_length * ray_struct->elemSize);
	return new_arr;
}
...
void UArray_reverse(T ray_struct)
{
	...
	{
		swap(UArray_at(ray_struct, i), UArray_at(ray_struct, ray_struct->len - 1 - i), elemSize);
	}
}
