...
void heapRemoveRoot(T heap_struct, int compare(void *p1, void *p2), int type)
{
	...
	swap_node(UArray_at(heap_struct->tree, 0), UArray_at(heap_struct->tree, heap_struct->size - 1), heap_struct->elemSize);
	heap_struct->size--;
...
// type == 1 for max heap
// type == 0 for min heap
void heapify_up(UArray_T tree, int nodeIndex, int compare(void *p1, void *p2), int type)
{
	assert(tree && compare);
	assert(nodeIndex >= 0);
	assert(type == 0 || type == 1);
